...
if (!validatePackage(pkg)) {
  console.error(
    "Error package.json is not valid. Check the config.docker.imageName key exists"
  );
  process.exit(1);
}

var build = handleSpawnStream(
  spawn("docker", ["build", "-t", imageNameAndVersion, "."])
);
...
build.on("close", function (code) {
  if (code !== 0) {
    ...
    var push = handleSpawnStream(
      spawn("docker", ["push", imageNameAndVersion])
    );
...
function handleSpawnStream(spawnStream) {
  ...
  spawnStream.on("error", function (err) {
    console.error(
      "Failed to start child process. Is Docker installed and available on PATH?"
    );
    process.exit(1);
  });
