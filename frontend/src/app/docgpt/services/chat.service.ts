...
import { BehaviorSubject, Observable, concat, concatMap, flatMap, of, switchMap, tap } from 'rxjs';
...
export class ChatService {
  ...
  private $queryBeingPrecessed = new BehaviorSubject<string | undefined>(undefined)
  constructor(
  ...
  public onQueryProcessingChange(): Observable<string | undefined> {
    ...
    public getChatById(projectId: number, chatId: number): Observable<Chat> {
      return this.httpClient.get<Chat>(`${this.url}/projects/${projectId}/chats/${chatId}`)
    }    

    public ask(chatId: number, query: string): Observable<Chat> {
      this.$queryBeingPrecessed.next(query)
      return this.httpClient.post<Chat>(`${this.url}/llm/ask`, {chatId: chatId, query: query}).pipe(
  ...
  public deleteChat(projectId: number, chatId: number): Observable<Project[]> {
    return this.httpClient.delete<Project[]>(`${this.url}/projects/${projectId}/chats/${chatId}`)    
    .pipe(
   ...
   public createNewChat(projectId: number, newChat: Partial<Chat> ) {
    return this.httpClient.post<Project[]>(`${this.url}/projects/${projectId}/chats`, newChat)    
    .pipe(
      tap(p => this.projectService.updateProjectList(p))
    )
   }
  ...
  public getChat(projectId: number, chatId: number): Observable<Chat> {
    return this.httpClient.get<Chat>(`${this.url}/${projectId}/chats/${chatId}`)   
  }
