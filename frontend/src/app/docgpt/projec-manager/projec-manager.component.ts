...
export class ProjecManagerComponent implements OnInit {
  ...
  languages = [{id:0, name: 'English'},{ id:1, name: 'Français'}]
  selectedLanguage = {id:0, name: 'English'}
  selectedModel = {id:0, name: 'gpt-3.5-turbo'}
  models = [{id:0, name: 'gpt-3.5-turbo'},{ id:1, name: 'gpt4'}]
    ...
    this.projectService.onProjectListChange().subscribe(projects=> {
      ...
      projects.forEach(p => {
        ...
        p.chats.forEach(c => {
          ...
          children.push({...newChildrenNode, expanded: this.isAutoExpanded(newChildrenNode)})
        })
  ...
  isAutoExpanded(t: TreeNode): boolean {
    return (t.children !== undefined) && (t.data.id === Number(this.currentProjectId))
  }
  ...
  deleteProject() {
    this.confirmationService.confirm({
      ...
      rejectLabel: 'Annuler',
      accept: () => {
          this.projectService.deleteProject(this.selectedNode.data)
          .subscribe(() => {
            this.messageService.add({ severity: 'info', summary: 'Confirmed', detail: 'Projet supprimé' })
            if (this.selectedNode.data.id === Number(this.currentProjectId)) this.router.navigate([""])
          })
      },
  ...
  onConfirmCreate() {
    this.projectService.createNewProject(this.newProjectName).subscribe(projects=> {
      this.newProjectName = ''
      this.createProjectVisible = false
      this.messageService.add({ severity: 'info', summary: 'Confirmed', detail: 'Projet créé' })
  ...
  onConfirmCreateChat() {
    this.chatService.createNewChat(this.selectedNode.data.id, {name: this.newChatName, settings: {model: this.selectedModel, language: this.selectedLanguage}}).subscribe((projects)=> {
      this.newChatName = ''
      this.createChatVisible = false
      this.messageService.add({ severity: 'info', summary: 'Confirmed', detail: 'Nouvelle conversation créée' })

      const targetProject = projects.find(p => p.id === this.selectedNode.data.id)
      if (targetProject !== undefined) {
        const targetChat = targetProject.chats[targetProject.chats.length -1]
        this.router.navigate(['project', targetProject.id, 'chat', targetChat.id ])
      }
  ...
  deleteChat() {
    this.confirmationService.confirm({
      ...
      rejectLabel: 'Annuler',
      accept: () => {
        this.chatService.deleteChat(this.selectedNode.parent.data.id, this.selectedNode.data.id)
        .subscribe(() => {
          this.messageService.add({ severity: 'info', summary: 'Confirmed', detail: 'Conversation supprimée' })
          if (this.selectedNode.data.id === Number(this.currentChatId)) this.router.navigate(["project", this.currentProjectId])
        })
      },
      reject: (type: ConfirmEventType) => {}
    })
      ...
      this.items = [
        {
          ...
          command: () => this.router.navigate(['project', this.selectedNode.data.id])
        },
        {
      ...
      this.items = [
        {
          label: 'Ouvrir',
          icon: 'pi pi-fw pi-sync',
          command: () => this.router.navigate(['project', this.selectedNode.parent.data.id, 'chat', this.selectedNode.data.id])
        },
        {
          label: 'Renommer',
          icon: 'pi pi-fw pi-pencil'
